...
export async function listFees(req: AuthRequest, res: Response) {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error listing fees.' });
  }
}
...
export async function createCategory(req: AuthRequest, res: Response) {
  try {
    ...
    if (!name || amount === undefined || !periodicity) {
      return res.status(400).json({ error: 'Name, amount, and periodicity are required.' });
    }
...
export async function allocateFee(req: AuthRequest, res: Response) {
  try {
    ...
    const { fee_category_id, class_id, student_id, start_date, end_date } = req.body;

    if (!fee_category_id || !start_date || !end_date) {
      return res.status(400).json({ error: 'Fee Category ID, start date, and end date are required.' });
    }
    ...
    return res.status(500).json({ error: 'Internal server error allocating fee.' });
  }
}
...
export async function listPayments(req: AuthRequest, res: Response) {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error listing payments.' });
  }
}
...
export async function collectPayment(req: AuthRequest, res: Response) {
  try {
    ...
    const { student_id, fee_allocation_id, amount_paid, method, period_identifier } = req.body;

    if (!student_id || !fee_allocation_id || amount_paid === undefined || !method || !period_identifier) {
      return res.status(400).json({ error: 'All fields are required.' });
    }

    const receiptNumber = 'REC-' + crypto.randomBytes(4).toString('hex').toUpperCase();
    ...
    return res.status(500).json({ error: 'Internal server error collecting payment.' });
  }
}
...
export async function downloadReceipt(req: Request, res: Response) {
  try {
    ...
    doc
      .text(`Receipt Number: ${payment.receiptNumber}`, 350, 50, { align: 'right' })
      .text(`Date: ${new Date(payment.paymentDate).toLocaleDateString()}`, 350, 65, { align: 'right' })
      .text(`Payment Mode: ${payment.method.toUpperCase()}`, 350, 80, { align: 'right' });
    ...
    doc
      .fontSize(10)
      .font('Helvetica')
      .text(`School Academic Fees - ${payment.feeAllocation.feeCategory.name}`, 50, 250)
    ...
    doc
      .fontSize(8)
      .fillColor('#9ca3af')
      .text('Thank you for your payment. This is an electronically generated receipt.', 50, 400, { align: 'center' });

    doc.end();
  } catch (err) {
    console.error('Download receipt error:', err);
    return res.status(500).json({ error: 'Internal server error generating receipt PDF.' });
  }
}
