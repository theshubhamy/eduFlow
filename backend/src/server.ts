...
app.get('/api/dashboard', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const classesCount = await prisma.schoolClass.count({ where: { schoolId } });
    ...
    const revenueCollected = totalRevenueResult._sum.amountPaid?.toNumber() || 0;
...
app.patch('/api/teams/:teamId/members/:memberUserId', requireAuth, updateMemberRole);
app.delete('/api/teams/:teamId/members/:memberUserId', requireAuth, removeMember);
app.post('/api/teams/:teamId/invitations', requireAuth, inviteMember);
app.delete('/api/teams/:teamId/invitations/:inviteCode', requireAuth, cancelInvitation);
